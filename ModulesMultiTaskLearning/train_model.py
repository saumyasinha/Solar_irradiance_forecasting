...
def train(X_train, y_train, X_valid, y_valid, n_tasks, input_size, hidden_sizes, task_specific_hidden_sizes, folder_saving, model_saved, n_epochs, lr, batch_size, weight_decay, soft_loss_weight = 0):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for minibatch in minibatches:
            ...
            output = model(data)
            # output, soft_loss = model(data) #in case of soft sharing
            # print(output.shape)
            total_loss = []
            ...
            loss = sum(total_loss)
            # loss = (sum(total_loss))+soft_loss_weight*soft_loss #in case of soft sharing
            ...
            # train_soft_loss= soft_loss.item() #in case of soft sharing
        ...
        train_loss = train_loss/len(minibatches)
        ...
        for n in range(n_tasks):
            task_specific_train_loss_list[n].append(task_specific_train_loss[n]/len(minibatches))
