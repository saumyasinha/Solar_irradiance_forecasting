import torch
from ModulesLearning.ModuleLSTM.Model import quantileLSTM,TransAm
import numpy as np
...
def test_transformer(quantile, X_valid, y_valid, X_test, y_test, n_timesteps, n_features, folder_saving, model_saved, n_outputs = 1):
    ...
    point_foreaster = TransAm(n_features, n_timesteps, folder_saving, model_saved, quantile, outputs=n_outputs,
                              valid=True)
    ...
    valid_crps, test_crps = 0.0, 0.0
    if quantile:
        test_crps = point_foreaster.crps_score(y_pred, y_test, np.arange(0.05, 1.0, 0.05))
        y_pred = y_pred[:,9]

        if X_valid is not None:
            valid_crps = point_foreaster.crps_score(y_valid_pred, y_valid, np.arange(0.05, 1.0, 0.05))
            y_valid_pred = y_valid_pred[:,9]
